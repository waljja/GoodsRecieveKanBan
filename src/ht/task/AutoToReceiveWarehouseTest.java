...
public class AutoToReceiveWarehouseTest {
    @Autowired
        ...
        try {
            ...
            if( (nowDayTime.substring(11).compareTo("08:00") >0 && nowDayTime.substring(11).compareTo("12:00") <=0 )
                ...
                PreparedStatement pstmtDB2 = connDB.prepareStatement("select * from ToReceiveWarehouse where ReturnWarehouseTime <>'' " +
                        " and GRN=? ");
                // vendorrid
                // 测试定时器是否启动两次，暂时去掉 '1100',
                rs = vpsDB.executeQuery("select grn, partNumber, printQTY, rid, plent from vendorrid " +
                        " where convert(varchar(10),GRNDATE,23) between '"+nowDay_2+"' and '"+nowDay+"' and UpAegis='pass' and plent in ('1200','5000')  ");
                ...
                rs = vpsDB.executeQuery("select grn, partNumber, printQTY, rid, plent from pcbvendorrid " +
                        " where convert(varchar(10),GRNDATE,23) between '"+nowDay_2+"' and '"+nowDay+"' and UpAegis='pass' and plent in ('1200')  ");
                ...
                for (int j = 0; j < list.size(); j++) {
                    ...
                    commonsLog.info("list" + (j + 1) + ":" + list.get(j).getUID());
    ...
    public static void main(String[]args) throws Exception{
        new AutoToReceiveWarehouseTest().execute();
    }
}