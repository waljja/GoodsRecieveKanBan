 ...
 *
 */
@Controller("ToReceiveWarehouseBAction")
@SuppressWarnings("unchecked")
public class ToReceiveWarehouseBAction extends ActionSupport{
    @Autowired
    private IToReceiveWarehouseBService trWarehouseBService;
    
    public String doList()throws Exception {
        //doSaveRecords();
    	//System.out.println("ToReceiveWarehouseBAction refresh time: "+ new Date());
        List<ToReceiveWarehouseB> list = trWarehouseBService.findAllToReceiveWarehouseB();    
        if (list!=null&&list.size()>0) {
            Map request = (Map) ActionContext.getContext().get("request");
            request.put("toReceiveWarehouseBList", list); 
        }
        getTotalSummary();
        list = null;
        return "tolist";
    ...
    public void doSaveRecords() throws Exception {
        ...
        ArrayList<ToReceiveWarehouseB> list = new ArrayList<ToReceiveWarehouseB>();
        //vendorrid
        ...
        for(String key : rsMap.keySet()) {
            ...
            ResultSet rsA = conMes.executeQuery(" select top 1 FRB.Name, II.StockLocation, DATEADD(HOUR,8,IIH.TimePosted_BaseDateTimeUTC) AS 'localtime' " +
                    ...
                    " where II.Identifier in ("+temp[3]+") and FRB.Name like '%QM%' and PMD.MeasurementType ='122'" +
    ...
    public void ListSort(List<ToReceiveWarehouseB> list) { 
        Collections.sort(list,new Comparator<ToReceiveWarehouseB>(){          
            public int compare(ToReceiveWarehouseB o1,
                    ToReceiveWarehouseB o2) {
